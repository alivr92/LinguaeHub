...
export function validateStep3() {
    ...
    if (cards.length > maxSkills) {
        showBootstrapAlert22(`Maximum ${maxSkills} skills allowed`, 'danger', 5000);
        return false;
    }
...
function createExistingFileDisplay(type, inputElement, labelElement, card, isIntroVideo = false) {
    ...
    changeBtn.onclick = () => {
        ...
        inputElement.onchange = () => {
            ...
            handleFileUploadForRow(type, inputElement, labelElement, card, {
                uSkillId,
 ...
 */
function initSelectsForCard1(card) {
        ...
        if (skillSelect && !skillSelect.hasAttribute('data-choicejs')) {
            new Choices(skillSelect, {
                ...
                classNames: {
                    ...
                    listDropdown: 'choices__list--dropdown dropdown-zindex',
                    item: 'choices__item'
                }
            });
...
// document.getElementById('applicant_video_intro').addEventListener('change', function(e) {
//     const fileNameDisplay = document.getElementById('intro-video-filename');
//     const container = document.querySelector('.file-upload-area');
//
//     if (this.files && this.files[0]) {
//         const fileName = this.files[0].name;
//         fileNameDisplay.textContent = fileName;
//         container.classList.add('video-uploaded');
//     } else {
//         fileNameDisplay.textContent = '';
//         container.classList.remove('video-uploaded');
//     }
// });
//
// // If you need to show existing video
// function showExistingVideo(fileName) {
//     const fileNameDisplay = document.getElementById('intro-video-filename');
//     const container = document.querySelector('.file-upload-area');
//
//     fileNameDisplay.textContent = fileName;
...
//--------------===============================================================================================
//_main
function addExistedSkillCard_main(uSkillId, skillId, levelId, video, certificate) {
    if (!template) {
        console.error('Template not found!');
        return;
    }
    ...
    addedCard.classList.add('skill-card');
    addedCard.dataset.cardId = uSkillId;

    // Track this as an existing skill
    trackers.skills.existed.push(uSkillId);

    // Set skill and level values
    const skillSelect = addedCard.querySelector('[name="skill"]');
    const levelSelect = addedCard.querySelector('[name="level"]');
    ...
    setupFileInputs(addedCard, uSkillId, uSkillId);


    // Display existing files if present
    const videoInput = addedCard.querySelector('[name^="skill_video"]');
    const certInput = addedCard.querySelector('[name^="certificate"]');
    const videoLabel = addedCard.querySelector('.label_skill_video');
    const certLabel = addedCard.querySelector('.label_skill_certificate');

    if (video && videoInput && videoLabel) {
        createExistingFileDisplay('video', videoInput, videoLabel, addedCard);
    }

    if (certificate && certInput && certLabel) {
        createExistingFileDisplay('certificate', certInput, certLabel, addedCard);
    }

    // Remove button logic
    setupRemoveButton(addedCard, uSkillId);
...
//_main
function addBlankSkillCard_main() {
    if (checkSkillLimit()) return;
    const newCard = template.content.cloneNode(true);
    container.appendChild(newCard);
    const addedCard = container.lastElementChild;

    const cardId = Date.now().toString(); // Ensure string ID for consistency

    const card = createSkillCard(cardId, newCard);
    setupFileInputs(card, cardId);
    setupRemoveButton(card, cardId);
    container.appendChild(card);
    ...
    initSelects();
    initTooltips();
    return cardId; // Return the new card ID

}
//_main
function createSkillCard_main(cardId, templateClone) {
    const card = document.createElement('div');
    card.className = 'skill-card';
    card.innerHTML = templateClone.querySelector('div').innerHTML;
    card.dataset.cardId = cardId;
...
//include enhanced styling
/**
 * Creates a new skill card with common setup
 * @param {string} cardId - Unique ID for the card
 * @param {boolean} isExisting - Whether this is an existing skill
 * @param {Object} skillData - Optional skill data (for existing skills)
 * @returns {HTMLElement} The created card element
 */
function createSkillCard2(cardId, isExisting = false, skillData = null) {
    if (!template) {
        console.error('Template not found!');
        return null;
    }
    ...
    const card = document.createElement('div');
    card.className = 'skill-card';
    card.innerHTML = newCard.querySelector('div').innerHTML;
    card.dataset.cardId = cardId;
    ...
    if (skillData) {
        const skillSelect = card.querySelector('[name="skill"]');
        const levelSelect = card.querySelector('[name="level"]');
        if (skillSelect) skillSelect.value = skillData.skillId;
        if (levelSelect) levelSelect.value = skillData.levelId;
    }
...
function handleFileUploadForRow(fileType, fileInput, fileLabel, cardElement, options = {}) {
...
// =============================================
function setupFileInputs_main(card, cardId, uSkillId = null) {
    const videoInput = card.querySelector('[name^="skill_video"]');
    const certInput = card.querySelector('[name^="certificate"]');
    const videoLabel = card.querySelector('.label_skill_video');
    const certLabel = card.querySelector('.label_skill_certificate');
    ...
    if (videoInput && videoLabel) {
        ...
        videoInput.addEventListener('change', () => {
            // For new skills, always mark as uploaded
            const isNewSkill = trackers.skills.new.includes(cardId);
            handleFileUploadForRow('video', videoInput, videoLabel, card, {
                uSkillId: cardId,
                isNew: isNewSkill
            });
        });
    ...
    if (certInput && certLabel) {
        ...
        certInput.addEventListener('change', () => {
            // For new skills, always mark as uploaded
            const isNewSkill = trackers.skills.new.includes(cardId);
            handleFileUploadForRow('certificate', certInput, certLabel, card, {
                uSkillId: cardId,
                isNew: isNewSkill
            });
        });
...
function setupRemoveButton_main(card, cardId) {
    console.log('setupRemoveButton');
    card.querySelector('.remove-skill')?.addEventListener('click', function () {
        console.log('setupRemoveButton CLICKED!');

        if (!confirm('Are you sure you want to remove this skill?')) return;

        // If this is an existing skill (not new), add to removed skills
        //Note 1: all files (video/cert) which relates to this card should be mark to delete from storage
        //Note 2: We don't need to track if a cardId was not belong to any existed skill!
        if (trackers.skills.existed.includes(cardId)) {
            updateTracker('video', 'removed', cardId, true);
            updateTracker('certificate', 'removed', cardId, true);
            updateTracker('skills', 'removed', cardId, true);
        ...
        //         trackers[type][action] = trackers[type][action].filter(id => id !== cardId);
        //     });
        // });
        //---------------------------------------------------------------------------------------------
...
//===============================================================================================
// =============================================
// MAIN FUNCTIONS - REFACTORED AND WORKING
// =============================================

/**
 * Creates and returns a new skill card element
 * @param {string} cardId - Unique ID for the card
 * @param {boolean} isExisting - Whether this is an existing skill
 * @param {Object} skillData - Optional skill data (for existing skills)
 * @returns {HTMLElement} The created card element
 */
function createSkillCard_new(cardId, isExisting = false, skillData = null) {
    if (!template) {
        console.error('Template not found!');
        return null;
    }
    ...
    const card = templateContent.querySelector('.skill-card');

    // Set card ID and data attributes
    card.dataset.cardId = cardId;
 ...
 * @param {string|null} certificate - Certificate URL if exists
 */
function addExistedSkillCard_new(uSkillId, skillId, levelId, video, certificate) {
    const card = createSkillCard(uSkillId, true, {skillId, levelId});
    if (!card) return;
 ...
 */
function addBlankSkillCard_new() {
    if (checkSkillLimit()) return null;
    ...
    // Initialize Choices.js and setup inputs
    initSelectsForCard(card);
    setupFileInputs(card, cardId);
    setupRemoveButton(card, cardId);
...
/**
 * Initializes Choices.js for select elements in a specific card
 * @param {HTMLElement} card - The card element
 */
function initSelectsForCard_new(card) {
    if (typeof Choices !== 'undefined') {
        const skillSelect = card.querySelector('[name="skill"]');
        const levelSelect = card.querySelector('[name="level"]');
        ...
        if (skillSelect && !skillSelect.hasAttribute('data-choicejs')) {
            new Choices(skillSelect, {
                ...
                classNames: {
                    ...
                    listDropdown: 'choices__list--dropdown bg-white border shadow-sm',
                    item: 'choices__item bg-light-hover',
                    itemSelectable: 'choices__item--selectable',
                    itemDisabled: 'choices__item--disabled',
                    placeholder: 'choices__placeholder text-muted'
                }
            });
            skillSelect.setAttribute('data-choicejs', 'true');
        }
