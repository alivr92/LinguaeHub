...
class TutorURLTests(TestCase):
    ...
    def test_seo_url(self):
        url = reverse('provider:tutor_detail', kwargs={'slug': self.tutor.slug})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_short_url_redirect(self):
        url = reverse('provider:tutor_short', kwargs={'public_id': self.tutor.public_id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 301)  # Permanent redirect

    def test_legacy_url_redirect(self):
        url = reverse('provider:tutor_legacy', kwargs={'pk': self.tutor.pk})
        response = self.client.get(url)
