...
function validatePhoneNumber() {
    ...
    const isValid = /^\d{8,15}$/.test(phoneNumber);

    if (phoneNumber && !isValid) {
        phoneInputWrapper.parentElement.classList.add('invalid');
        return false;
    } else {
        phoneInputWrapper.parentElement.classList.remove('invalid');
        return true;
    }
}
...
//         fullPhoneNumber.value = selectedCountry.dial + phoneNumberInput.value;
//         alert(`Form submitted with phone number: ${fullPhoneNumber.value}`);
