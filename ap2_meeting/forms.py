...
class AppointmentSettingForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['session_length', 'provider_timezone', 'week_start', 'session_type']
        widgets = {
            'session_length': forms.Select(attrs={'class': 'form-control form-select'}),
            'provider_timezone': forms.Select(attrs={'class': 'form-control form-select'}),
...
class SessionForm(forms.ModelForm):
    class Meta:
        model = Session
        fields = [
            'subject', 'session_type', 'tutor', 'students',
    ...
    def clean(self):
        ...
        if start_session_utc and end_session_utc and tutor:
