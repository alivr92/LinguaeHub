...
class CombinedProfileForm(forms.Form):
    ...
    # Student fields
    major = forms.CharField(max_length=100, required=False, widget=forms.TextInput(attrs={
        'class': 'form-control',
        'placeholder': 'Major',
    }))
    ...
    def __init__(self, *args, **kwargs):
        # Extract instance data for User, UserProfile, and Student
        ...
        if self.student_instance:
            ...
            student_fields = ['major', 'interests']
            for field in student_fields:
                self.fields[field].initial = getattr(self.student_instance, field)
    ...
    def save(self, user):
        ...
        student, created = Student.objects.update_or_create(profile=profile, defaults=student_data)

        # Handle many-to-many fields
        student.interests.set(self.cleaned_data['interests'])
        profile.lang_speak.set(self.cleaned_data['lang_speak'])

        return user_instance, profile, student
